...
class GyazoManager: NSObject {
    ...
    func startOauth() {
        ...
        let requestURL = URL(
            string:
                "\(host)/oauth/authorize?client_id=\(clientId!)&redirect_uri=\(redirectUri)&response_type=code"
        )
    ...
    private func getAccessToken(code: String) {
        ...
        let url = URL(
            string:
                "\(host)/oauth/token?client_id=\(clientId!)&redirect_uri=\(redirectUri)&code=\(code)&grant_type=authorization_code&client_secret=\(secretKey!)"
        )!
        ...
        let task = URLSession.shared.dataTask(with: request) { [self] data, response, error in
            ...
            do {
                let items = try JSONSerialization.jsonObject(with: responseData) as! [String: Any]
    ...
    private func postImageWithEasyAuth(image: NSImage) {
        ...
        let url = URL(
            string:
                "https://upload.gyazo.com/api/upload/easy_auth?client_id=\(clientId!)&referer_url=\(redirectUri)"
        )!
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
    ...
    private func warningAlert() {
        ...
        alert.informativeText =
            "GYAZO_CLIENT_ID or GYAZO_SECRET is empty.\nPlease check your scheme's environment."
